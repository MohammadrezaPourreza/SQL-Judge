"""Utility functions for retrieving information from a database."""
...
def get_db_all_tables(db_path: str) -> List[str]:
  ...
  try:
    ...
    raw_table_names = execute_sql(db_path, query)
    return [
    ...
    logging.exception('Error in get_db_all_tables: %s', e)
    raise e
...
def get_table_all_columns(db_path: str, table_name: str) -> List[str]:
  ...
  try:
    ...
    logging.exception(
        'Error in get_table_all_columns: %s\nTable: %s', e, table_name
    )
    raise e
...
def get_db_schema(db_path: str) -> Dict[str, List[str]]:
  ...
  try:
    ...
    logging.exception('Error in get_db_schema: %s', e)
    raise e